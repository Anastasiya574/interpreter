...
void init_labels(std::vector<Lexem *> &infix, int row) {
	...
	//std::cout << "endofinitlabels" << std::endl;
}
...
int get_value(Lexem *leftlex, Lexem *rightlex, Function **function, OPERATOR opertype) {
	...
	switch (opertype) {
			case ASSIGN:
				...
				else if (Array* ptr = dynamic_cast<Array*>(leftlex)) {
					int tmp;
					if (Number* ptr = dynamic_cast<Number*>(rightlex)) {
						tmp = ((Number*)rightlex)->get_value();
					}
					else {
						tmp = ((Array*)rightlex)->get_value();
					}
					((Array*)leftlex)->set_value(tmp);
					return tmp;
				} else {
...
int evaluate_postfix(std::vector<std::vector<Lexem *>> &postfix, int row, Function **function_field, Number **result) {
	...
	for (auto &element: postfix[row]) {
		...
		if(element->get_type() == SIZE) {
			right = stack.back();
			stack.pop_back();
			left = stack.back();
			stack.pop_back();
			ArrayTable[left->get_name()] = new int[right->get_value()];	
			stack.push_back(nullptr);
			continue;
		}
		if(element->get_type() == LSQRBRACKET) {
			right = stack.back();
			stack.pop_back();
			left = stack.back();
			stack.pop_back();
			stack.push_back(new Array(left, right));
			continue;
        }
